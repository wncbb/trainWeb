...
(function (root, factory) {
        ...
        return function(planet) {
            planet.onDraw(function() {
                planet.withSavedContext(function(context) {
                    ...
                    var tmp=context.createRadialGradient(1000, 320, 0, 1000, 320, 800);
                    tmp.addColorStop(0, '#444');
                    tmp.addColorStop(0.8, '#111');

                    planet.path.context(context)({type: 'Sphere'});
                    //context.fillStyle = config.fill || 'black';
                    context.fillStyle=tmp;
                    context.shadowBlur=config.shadowBlur||160;
                    context.shadowColor=config.shadowColor||'#1c3651';
                    context.fill();
                });
            });
        };
    ...
    //planetaryjs.plugins.oceans2 = function(config) {
    //    return function(planet) {
    //        planet.onDraw(function() {
    //            planet.withSavedContext(function(context) {
    //                context.beginPath();
    ...
    planetaryjs.plugins.myWorld = function(config) {
        var myWorld=null;
        var baseColor = "#226622";
        //var baseColor="#880000";
        var countryColors=[];
        var diff=0;
        var baseR=0;
        var baseG=0;
        ...
        return function(planet) {
            var countries = null;
            planet.onInit(function(done) {
                d3.json("./static/json/world2.geo.json", function(error, data){
                    myWorld=data;
                    ...
                    //console.log("baseR:"+baseR);
                    //console.log("baseG:"+baseG);
                    //console.log("baseB:"+baseB);
                    for(var i=0; i<num; ++i)
                    {
                        var pre=parseInt(i/3);
                        var apd=parseInt(i-pre*2);
                        //console.log("i:"+i+" pre:"+pre+" apd:"+apd);
                        var rstR=baseR+pre;
                        var rstG=baseG+apd;
                        var rstB=baseB+pre;
