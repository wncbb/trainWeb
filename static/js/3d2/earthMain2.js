...
function initPlanet (inArg){
  ...
  planet.projection.rotate([240, -30, 0]);
  planet.draw(canvas);
  ...
  d3.select("#addPing").on("click", function(){
  ...
  * */
  retObj.attack=function (inObj){
    ...
    planet.plugins.lines.add({
      ...
      lat1: inObj.dst.lat,
      lng1: inObj.dst.lng,
      lineWidth: inObj.line.lineWidth||1
    });
    setTimeout(function(){
      planet.plugins.pings.add({
        // Here we use the `angles` and `colors` scales we built earlier
  ...
  //  var colorData = this.getPixelColor(e.clientX, e.clientY);
  ...
  $("#mapCanvas").mousedown(function(e) {
    ...
    if(stationColor.hasOwnProperty(colorData.hex) && (stationInfo.hasOwnProperty(stationColor[colorData.hex]))){
      console.log(stationInfo[stationColor[colorData.hex]].name);
...
function simulationAttack(inPlanet){
